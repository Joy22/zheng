...
public class SSOController extends BaseController{
     ...
     * 全局会话key列表
     */
    private final static String ZHENG_UPMS_SERVER_SEIION_IDS = "zheng-upms-server-session-ids";
    ...
    public Object login(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap){
        ...
        if(StringUtils.isBlank(hasCode)){
            ...
            RedisUtil.lpush(ZHENG_UPMS_SERVER_SEIION_IDS, sessionId);
            // 默认验证账号密码正确，创建code
            String code = UUID.randomUUID().toString();
            // 全局会话的code
            RedisUtil.set(ZHENG_UPMS_SERVER_SESSION_ID + "_" + sessionId, code, (int) (session.getTimeout() / 1000));
            // code校验值
            RedisUtil.set(ZHENG_UPMS_SERVER_CODE + "_" + code, code, (int) (session.getTimeout() / 1000));
        }
        // 回跳登录前地址
        String backurl = request.getParameter("backurl");
        if(StringUtils.isBlank(backurl)){
